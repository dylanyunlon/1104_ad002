...
#define CHECK_STATE_PERIOD    10000
#define CHECK_CYCLE_PEPIOD	  10000

enum {
	E_MSG_CHECK_STATE = 0x10,
};
...
static int _read_driver_state() {
	char str[32] = {0};
	FILE *driver_state = fopen("/sys/module/aic8800_fdrv/parameters/driver_state", "r");
    if (!driver_state) {
        LOGD("Failed to open driver_state file");
        return 0;
    }

    // 更安全的读取方式：最多读取31字符确保字符串终止
    if (!fgets(str, sizeof(str), driver_state)) {
        LOGD("Failed to read driver_state");
        fclose(driver_state);
        return 0;
    }
...
static void _message_handler(const fy::message *msg, const void *user_data) {
	...
	if (msg->what == E_MSG_CHECK_STATE) {
		if (_read_driver_state() == 0) {return;}		// 正常
		else {											// 异常 恢复
			driver_debug++;
			if (driver_debug >= 5) {
//				LOGD("[net] sys::setting::get_link_mode() %d", sys::setting::get_link_mode());
				driver_debug = 0;
...
void init() {
	...
	system("echo 5242880 > /proc/sys/net/core/wmem_max");
...
//		if (sys::setting::get_link_mode() == E_LINK_MODE_MIRACAST) {
	...
	_s_handler.schedule(E_MSG_CHECK_STATE, CHECK_CYCLE_PEPIOD, CHECK_STATE_PERIOD);
}
