...
static bool _otp_read_data_core(uint8_t *data, uint32_t size, ESecDataPage page = E_SECDATA_PAGE_USER) {
	Mutex::Autolock _l(_s_lock);
	for (int i = 0; i < TRY_COUNT; ++i) {
		if (SECURITYMANAGER->readSecData(data, size, page) == (int) size) {
			return true;
		}
...
static bool _otp_write_data_core(uint8_t *data, uint32_t size, ESecDataPage page = E_SECDATA_PAGE_USER) {
	Mutex::Autolock _l(_s_lock);
	if (SECURITYMANAGER->isSecDataLock(page)) {
		LOGE("otp page %d is lock, can't write data!!!\n", page);
		return false;
	}

	for (int i = 0; i < TRY_COUNT; ++i) {
		if (SECURITYMANAGER->writeSecData(data, size, page) == (int) size) {
			return true;
		}

		LOGE("page%d _otp_write_data_core err %d\n", page, i);
		DELAY(50);
	}
	return false;
}

bool otp_read_data(int offset, uint8_t *data, uint32_t size, ESecDataPage page) {
	if (offset + size > MAX_DATA_SIZE) {
		return false;
	}

	uint8_t otp_data[MAX_DATA_SIZE];
	if (!_otp_read_data_core(otp_data, MAX_DATA_SIZE, page)) {
		return false;
	}
...
bool otp_write_data(int offset, uint8_t *data, uint32_t size, ESecDataPage page) {
	if (offset + size > MAX_DATA_SIZE) {
		return false;
	}
...
bool otp_load_data_to_file(int offset, uint32_t size, const char *file, ESecDataPage page) {
	uint8_t otp_data[size];
	if (!otp_read_data(offset, otp_data, size, page)) {
		LOGE("page%d:otp_read_data fail !!!\n", page);
		return false;
	}
