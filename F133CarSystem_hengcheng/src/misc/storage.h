 ...
 */
bool otp_read_data(int offset, uint8_t *data, uint32_t size, ESecDataPage page = E_SECDATA_PAGE_USER);

/**
 * @brief 往otp offset偏移位置写数据，数据最长256个字节
 */
bool otp_write_data(int offset, uint8_t *data, uint32_t size, ESecDataPage page = E_SECDATA_PAGE_USER);

/**
 * @brief 加载otp数据保存到文件
 */
bool otp_load_data_to_file(int offset, uint32_t size, const char *file, ESecDataPage page = E_SECDATA_PAGE_USER);
