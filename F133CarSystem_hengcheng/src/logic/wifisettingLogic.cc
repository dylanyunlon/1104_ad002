...
//	        sWifiInfos.assign(wifiInfos->begin(), wifiInfos->end());
	...
	virtual void notifySupplicantStateChange(int networkid,
		...
		case AUTHENTICATING: {
			Mutex::Autolock _l(sLock);
			sWifiChangeAps[bssid] = LTOV("sys_wifi_status_authenticating");
			break;
		}
		case ASSOCIATING:
			break;
		...
		case GROUP_HANDSHAKE: {
			Mutex::Autolock _l(sLock);
			sWifiChangeAps[bssid] = LTOV("sys_wifi_status_handshake");
			break;
		}
		case COMPLETED: {
			Mutex::Autolock _l(sLock);
...
static std::string getEncryptionInfo(const WifiInfo &wi) {
	...
	if (encryption.find("WPS") != std::string::npos) {
		info += "(WPS " + LTOV("sys_wifi_encryption_encrypted") + ")";
	}
	if (info.empty()) {
		info = LTOV("sys_wifi_encryption_open");
	}
...
static void onUI_init() {
	...
	mTextview17Ptr->setLongMode(ZKTextView::E_LONG_MODE_SCROLL_CIRCULAR);
	mTextview27Ptr->setLongMode(ZKTextView::E_LONG_MODE_SCROLL_CIRCULAR);
	mTextview19Ptr->setLongMode(ZKTextView::E_LONG_MODE_SCROLL_CIRCULAR);
	mButtonDisconnectPtr->setLongMode(ZKTextView::E_LONG_MODE_SCROLL_CIRCULAR);
	mButtonConnectPtr->setLongMode(ZKTextView::E_LONG_MODE_SCROLL_CIRCULAR);
	// 注意：按钮组件可能不支持 setLongMode 方法，如果报错请移除以下两行
//	但buttonshowpwd的dots生效了，textview无法生效
	mButtonShowPwdPtr->setLongMode(ZKTextView::E_LONG_MODE_SCROLL_CIRCULAR);
	mTextSsidPtr->setLongMode(ZKTextView::E_LONG_MODE_SCROLL_CIRCULAR);
	mTextSecTypePtr->setLongMode(ZKTextView::E_LONG_MODE_SCROLL_CIRCULAR);
...
static void obtainListItemData_ListViewWifiInfo(ZKListView *pListView,ZKListView::ZKListItem *pListItem, int index) {
	...
	if (WIFIMANAGER->isConnected() && (WIFIMANAGER->getConnectionInfo()->getBssid() == wi.getBssid())) {
		...
		pSubItem->setText(LTOV("sys_wifi_status_connected"));
	} else {
